from Basic import get_Markov_Order4
...
if __name__=='__main__':
    ...
    for Jsonfile in os.listdir(rootdir):
        ...
        for Jsondata in Jsondatas:
            ...
            Markov+=get_Markov_Order4(VideoTraffic, MaxNum=MaxNum, BoxSize=BoxSize)
        # Markov=Markov / len(Jsondatas)
        for i in range(MaxNum):
            for j in range(MaxNum):
                for k in range(MaxNum):
                    for l in range(MaxNum):
                        Markov[i][j][k][l]=Markov[i][j][k][l]/Markov[i][j][k][l].sum() if Markov[i][j][k][l].sum()>0 else 0
    ...
    for Jsonfile in os.listdir(rootdir):
        ...
        for Jsondata in Jsondatas:
            ...
            Sessions=Jsondata['Sessions']
            VideoTraffic, AudioTraffic = get_Traffic(Sessions=Sessions)
            Markov=get_Markov_Order4(Traffic=VideoTraffic, MaxNum=MaxNum, BoxSize=BoxSize)
    ...
    Name = '{0}order_{1}-{2}.json'.format(4, MaxNum, BoxSize)
