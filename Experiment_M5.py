from Basic import get_Markov_Order5
...
if __name__=='__main__':
    ...
    for Jsonfile in os.listdir(rootdir):
        ...
        for Jsondata in Jsondatas:
            ...
            Markov+=get_Markov_Order5(VideoTraffic, MaxNum=MaxNum, BoxSize=BoxSize)
                ...
                for k in range(MaxNum):
                    for l in range(MaxNum):
                        for m in range(MaxNum):
                            Markov[i][j][k][l][m]=Markov[i][j][k][l][m]/Markov[i][j][k][l][m].sum() if Markov[i][j][k][l][m].sum()>0 else 0
    ...
    for Jsonfile in os.listdir(rootdir):
        ...
        for Jsondata in Jsondatas:
            ...
            Sessions=Jsondata['Sessions']
            VideoTraffic, AudioTraffic = get_Traffic(Sessions=Sessions)
            Markov=get_Markov_Order5(Traffic=VideoTraffic, MaxNum=MaxNum, BoxSize=BoxSize)
    ...
    Name = '{0}order_{1}-{2}.json'.format(5, MaxNum, BoxSize)
